...
namespace mms
{
    struct PreMasterSecret
    {
        ...
        int32_t decode(uint8_t *data, size_t len)
        {
            ...
            int32_t consumed = client_version.decode(data, len);
            if (consumed < 0)
            {
                return -1;
            }
    ...
    struct ClientKeyExchange : public HandShakeMsg
    { // note:we only use rsa key exchange algorithm, so we only need to deal with rsa
